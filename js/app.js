    ...
    card.innerHTML = `
      ...
      <div class="body">
        ...
        <div class="resource-actions">
          ${item.type === 'game' || item.type === 'interdisciplinary'
            ...
            : `<a class="cta" href="${EMLS.url(item.lessonUrl || item.url || item.packUrl)}">📦 Отвори комплекта</a>`}
        </div>
      </div>`;
...
async function loadPublishedResources() {
  try {
    const response = await fetch(EMLS.url('data/resources.json') + '?v=8.8.6', { cache: 'no-store' });
    ...
    const published = resources
      ...
      .map(item => ({
        title: item.title,
        subject: item.subjectCode || String(item.subject || '').toLowerCase(),
        subjectLabel: item.subject || item.subjectCode || '',
        grade: `${item.grade}. клас`,
        gradeNumber: String(item.grade),
        description: item.description || '',
        icon: item.icon || '📘',
        theme: item.theme || 'teal',
        url: item.lessonUrl || '',
        lessonUrl: item.lessonUrl || '',
        packUrl: item.lessonUrl || '',
        tags: item.tags || [],
        featured: Boolean(item.featured),
        type: item.type || 'Учебен комплект',
        collection: item.type || 'Учебен комплект',
        section: item.type || '',
        objectives: item.objectives || [],
        materials: []
      }));

    const existingKeys = new Set(catalog.map(x => `${x.subject}|${x.gradeNumber}|${x.title}`));
    published.forEach(item => {
      const key = `${item.subject}|${item.gradeNumber}|${item.title}`;
    ...
    console.warn('EM Learning Studio: resources.json could not be loaded.', error);
  }
}

loadPublishedResources().finally(render);
